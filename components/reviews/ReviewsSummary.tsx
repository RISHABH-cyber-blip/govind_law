...
export default function ReviewsSummary() {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-black/80 border border-gold/30 rounded-2xl p-6 md:p-8 backdrop-blur-md shadow-2xl relative overflow-hidden"
    >
      {/* Soft background glow */}
      <div className="absolute -top-12 -right-12 w-40 h-40 bg-gold/10 rounded-full blur-3xl pointer-events-none" />

      <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
        {/* Left side: Rating count & stars */}
        <div className="flex items-center gap-5 text-center sm:text-left">
          <div className="flex flex-col items-center justify-center">
            <span className="font-serif text-5xl md:text-6xl font-bold text-gold leading-none tracking-tight">
              {displayRating}
            </span>
            <span className="text-xs text-gray-400 mt-1 font-sans">Out of 5.0</span>
          </div>

          <div className="border-l border-[#262626] pl-5">
            <div className="flex items-center gap-1 text-gold text-xl mb-1">
              {"★".repeat(5)}
            </div>
            <p className="text-gray-300 font-serif text-lg font-medium">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/reviews"
            className="btn-primary text-sm py-3 px-6 rounded-xl flex items-center gap-2 group cursor-pointer"
          >
